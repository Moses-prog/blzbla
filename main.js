   ...
   if (pairingCode && !XeonBotInc.authState.creds.registered) {
    ...
    XeonBotInc.ev.on('messages.upsert', async chatUpdate => {
        ...
        try {
            ...
            require("./techgod11")(XeonBotInc, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
...
XeonBotInc.ev.on("connection.update",async  (s) => {
        ...
        if (connection === "close") {
            ...
            if (lastDisconnect?.error?.output?.statusCode !== 401) {
                ...
                console.log('Authentication failed, please delete session folder and try again')
            }
        }
        if (connection == "open") {
            ...
            console.log(chalk.magenta(`${themeemoji} CREDIT: TechGodSir\n`))
        }
        if (
            connection === "close" &&
            lastDisconnect &&
